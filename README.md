...
> - Reports & Semantic models need to be promoted or curated.
> - The table in the semantic model needs to be named according to the DBX `catalog` `schema` `table` as to figure out which DBX tables it refers to. 
...
This lineage is super useful, it makes the following way easier.
 ...
 - Finding the source of a broken or incorrect report. Find (sometimes
   useless) tables in gold with no downstream Power BI reports. 
   
 - Figure out which reports access sensitive data (to implement increased
   governance on them). 
